...
void * handle_client(void *arg){
	...
	char *res = "HTTP/1.1 200 OK\r\nServer: Apache/2.4.46 (Ubuntu)\r\nAccept-Ranges: bytes\r\nVary: Accept-Encoding\r\nContent-Length: 123\r\nKeep-Alive: timeout=5, max=100\r\nConnection: Keep-Alive\r\nContent-Type: text/html\r\n\r\n<!DOCTYPE html><html><head><title>Networks Lab Exp 7</title></head><body><h1>Welcome to the Dept. of CSE!</h1></body></html>";
	
	char *res1 = "HTTP/1.1 200 OK\r\nServer: Apache/2.4.46 (Ubuntu)\r\nAccept-Ranges: bytes\r\nVary: Accept-Encoding\r\nContent-Length: 119\r\nKeep-Alive: timeout=5, max=100\r\nConnection: Keep-Alive\r\nContent-Type: text/html\r\n\r\n<!DOCTYPE html><html><head><title>Networks Lab Exp 7</title></head><body><h1>404 error: File not found</h1></body></html>";
	
	char res2[] = "HTTP/1.1 200 OK\r\nServer: Apache/2.4.46 (Ubuntu)\r\nConnection: Close\r\nContent-Type: text/html\r\n\r\n";
	...
	while(1){
		...
		if(buffer[5] == ' '){
			...
			printf("New Request from Client\n\n%s",buffer);
			send(client->sockfd, res1,strlen(res1),0);
			printf("Response sent to the client\n\n");
		}
		if(fp){
			char buffer1[500];
			while(fgets(buffer1,sizeof(buffer1)-1,fp))
				strcat(res2,buffer1);
			fclose(fp);
			send(client->sockfd,res2,strlen(res2),0);
			printf("html sent to client\n\n");
			break;
		}
